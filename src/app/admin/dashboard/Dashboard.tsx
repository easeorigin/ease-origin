...
export default function Dashboard() {
  ...
  const statCards = [
    { title: "Total Jobs", value: stats?.totalJobs || 0, icon: Briefcase, color: "text-blue-500", bg: "bg-blue-500/10" },
    { title: "Applications", value: stats?.totalApplications || 0, icon: FileText, color: "text-emerald-500", bg: "bg-emerald-500/10" },
    { title: "Case Studies", value: stats?.totalCaseStudies || 0, icon: Newspaper, color: "text-purple-500", bg: "bg-purple-500/10" },
    { title: "Contact Messages", value: stats?.totalContacts || 0, icon: Mail, color: "text-amber-500", bg: "bg-amber-500/10" },
  ];
            ...
            {stats?.recentApplications && stats.recentApplications.length > 0 ? (
              <div className="divide-y divide-border/50">
                {stats.recentApplications.map((app) => (
                  <div key={app.id} className="p-4 hover:bg-muted/30 transition-colors flex items-center justify-between">
                    <div>
                      <p className="font-medium text-sm text-foreground">{app.fullName}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">{app.jobTitle}</p>
                    </div>
                    <div className="text-right">
                      <Badge variant="outline" className="mb-1 capitalize text-xs px-2 py-0 h-5">
                        {app.status}
                      </Badge>
            ...
            {stats?.recentContacts && stats.recentContacts.length > 0 ? (
              <div className="divide-y divide-border/50">
                {stats.recentContacts.map((msg) => (
                  <div key={msg.id} className="p-4 hover:bg-muted/30 transition-colors flex items-center justify-between">
